...
export default function CategoriesPage() {
  ...
  function startEditCat(cat: Category) {
    setEditCat(cat);
    setCatForm({ name: cat.name, slug: cat.slug, is_active: cat.is_active });
  }
    ...
    <div className="pb-12 max-w-5xl mx-auto">
      ...
      >
        ...
        <div className="flex flex-wrap items-center gap-4">
          ...
          {editCat && (
            <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} onClick={() => { setEditCat(null); setCatForm({ name: "", slug: "", is_active: true }); }} className="bg-surface-container border border-outline-variant/50 text-on-surface-variant font-label font-bold px-6 py-3 rounded-xl text-sm">
              Cancel
            </motion.button>
          )}
        </div>
