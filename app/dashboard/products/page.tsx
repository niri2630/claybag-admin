...
export default function ProductsPage() {
  ...
  function startEdit(p: Product) {
    ...
    setForm({
      ...
      variant_mode_override: p.variant_mode_override === "option_dropdown" ? "option_dropdown" : null,
      ...
      curved_edge_price: p.curved_edge_price ?? null,
      hsn_code: p.hsn_code || "",
      ...
      is_swag_only: p.is_swag_only || false,
          ...
          {(editMode || !selected) && (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-surface-container-lowest rounded-[2.5rem] shadow-xl shadow-surface-variant/20 border border-outline-variant/30 overflow-hidden">
              ...
              <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                {/* Variant Mode override — for products like standees, photo frames, table-top displays
                    that should use a single dropdown selector instead of the variation matrix */}
                <div className="col-span-2 md:col-span-1">
                  <label className="font-label font-bold text-xs uppercase tracking-wider text-on-surface-variant block mb-2">
                  ...
                  <div className="flex gap-2">
                    <button type="button" onClick={() => setForm(f => ({ ...f, variant_mode_override: null, option_label: "" }))}
                  ...
                  <p className="text-[10px] text-on-surface/40 mt-1">
                    {form.variant_mode_override === "option_dropdown"
                      ? "Customer picks ONE option from a dropdown. Each option has its own price + MRP."
                      : "Default behaviour from category (matrix grid or chips)."}
                  </p>
                </div>
                ...
                {form.variant_mode_override === "option_dropdown" && (
                  <div className="col-span-2 md:col-span-1">
                    ...
                    <input
                      ...
                      className="w-full bg-surface-container border border-outline-variant/50 rounded-2xl px-4 py-3.5 text-on-surface font-medium focus:outline-none focus:ring-2 focus:ring-secondary-container transition-all"
                      placeholder="e.g. Size, Frame Style, Format"
                    />
                    <p className="text-[10px] text-on-surface/40 mt-1">Header shown above the dropdown on the product page.</p>
                  </div>
                )}
                ...
                <div className="col-span-2">
                  <label className="font-label font-bold text-xs uppercase tracking-wider text-on-surface-variant block mb-2">
                    <span className="material-symbols-outlined text-[14px] align-middle mr-1">smart_display</span>
                ...
                <div className="col-span-2">
                  <label className="font-label font-bold text-xs uppercase tracking-wider text-on-surface-variant block mb-2">
                    <span className="material-symbols-outlined text-[14px] align-middle mr-1">rounded_corner</span>
                    Curved-edge price (₹ / 100 cards)
                  </label>
                  <input
                    type="number"
                    min="0"
                    step="1"
                    value={form.curved_edge_price ?? ""}
                    onChange={e => setForm(f => ({ ...f, curved_edge_price: e.target.value === "" ? null : Number(e.target.value) }))}
                    placeholder="e.g. 25 — leave blank to hide the Curved Edge option"
                    className="w-full bg-surface-container border border-outline-variant/50 rounded-2xl px-4 py-3.5 text-on-surface font-medium focus:outline-none focus:ring-2 focus:ring-secondary-container transition-all"
                  />
                  <p className="text-xs text-on-surface-variant/70 mt-1.5">
                    Adds a &quot;Card Edge: Straight / Curved&quot; choice on the product page. Straight is free; choosing Curved adds this amount per 100-card pack (charged in the cart). The customer&apos;s choice shows on the order. Set it on business cards; leave blank elsewhere.
                  </p>
                </div>
                ...
                <div className="col-span-2 md:col-span-1 flex flex-col justify-center gap-4 bg-surface-container-low rounded-2xl p-5 border border-outline-variant/30">
                  ...
                  <div className="h-px bg-outline-variant/20 w-full" />
                  <label className="flex items-center justify-between cursor-pointer group">
                    <span className="font-label font-bold text-sm text-on-surface group-hover:text-primary transition-colors">Deal of the Month (Homepage hero deal &mdash; pick up to two)</span>
                    <input type="checkbox" checked={form.is_deal_of_month} onChange={e => setForm(f => ({ ...f, is_deal_of_month: e.target.checked }))} className="w-5 h-5 accent-secondary-container rounded" />
                  </label>
                  <div className="h-px bg-outline-variant/20 w-full" />
                  <label className="flex items-center justify-between cursor-pointer group">
                    <span className="font-label font-bold text-sm text-on-surface group-hover:text-primary transition-colors">24-Hour Delivery in Bangalore (Homepage express section &mdash; pick up to four)</span>
                    <input type="checkbox" checked={form.is_express_bangalore} onChange={e => setForm(f => ({ ...f, is_express_bangalore: e.target.checked }))} className="w-5 h-5 accent-secondary-container rounded" />
                  </label>
                  <div className="h-px bg-outline-variant/20 w-full" />
                  <label className="flex items-center justify-between cursor-pointer group">
                  ...
                  <div className="h-px bg-outline-variant/20 w-full" />
                  <label className="flex items-center justify-between cursor-pointer group">
                    <span className="font-label font-bold text-sm text-on-surface group-hover:text-primary transition-colors">Swag Box Only (shown in the hamper builder, hidden from the main catalog)</span>
                    <input type="checkbox" checked={form.is_swag_only} onChange={e => setForm(f => ({ ...f, is_swag_only: e.target.checked }))} className="w-5 h-5 accent-secondary-container rounded" />
                  </label>
                </div>
